...
def main() -> None:
    ...
    train_list = msc.build_data_list(all_rows, "train", roots, args.max_train_cases)
    val_list = msc.build_data_list(all_rows, "val", roots, args.max_val_cases)
    if not train_list:
    ...
    train_tf = msc.train_transforms(patch_size, spacing)
    val_tf = msc.val_transforms(spacing)
    ...
    if args.final_test:
        test_list = msc.build_data_list(all_rows, "test", roots, None)
        if not test_list:
            print("No test split rows.", file=sys.stderr)
            return
        msc.load_checkpoint(model, best_path, device)
