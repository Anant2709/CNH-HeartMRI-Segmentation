...
### Three follow-up experiments completed (Tasks 1–3)

These tasks were run after the baseline result on the primary split:

- Baseline (single split): internal val mean FG Dice ~**0.7193**, external test mean FG Dice ~**0.2967**.
- Motivation: quantify internal reliability, test mixed-site training behavior, and isolate whether the major drop is specifically external-domain related.

---

### Task 1 — Internal 5-fold CV (reliability of internal benchmark)

**Question this task answers**

- If we only change which internal patients are used for validation, how stable is the internal metric?
- This is an uncertainty/reliability experiment, not a domain-shift fix.

**Protocol**

- Split source: `scripts/build_splits.py --internal-cv-folds 5`
- Generated files: `reports/splits/internal_cv_fold_00.csv` ... `internal_cv_fold_04.csv`
- Training: Slurm array over folds using fold-specific `--split-csv`.
- Metric summarized from each fold run: `summary.json` key `best_val_mean_fg_dice`.

**Observed output**

- Fold scores: `0.733420`, `0.775842`, `0.690543`, `0.685807`, `0.782882`
- `n=5`, mean `0.733699`, std `0.040868`

**Interpretation**

- Internal performance estimate is **~0.734 ± 0.041** across fold assignments.
- This is close to (slightly above) the earlier single-split internal val (~0.719), so the baseline internal result is plausible and not obviously a one-off.
- Fold spread confirms internal case-mix sensitivity but not instability.

---

### Task 2 — Mixed-site 5-fold CV (internal + external pooled)

**Question this task answers**

- If train/val folds are built from pooled internal + external patients, how does validation behave under mixed-domain exposure?
- This probes mixed-domain training behavior; it is **not** equivalent to a strict external holdout test protocol.

**Protocol**

- Split source: `scripts/build_splits.py --cv-folds 5`
- Generated files: `reports/splits/mixed_site_cv_fold_00.csv` ... `mixed_site_cv_fold_04.csv`
- Training: Slurm array over mixed-site folds.
- Log health check:
  - `mcv-*.out` showed smooth loss decay and periodic validation every 5 epochs.
  - `mcv-*.err` showed warnings (MONAI orientation default notice + PyTorch deprecation), no fatal errors.

**Observed output**

- Fold scores: `0.660376`, `0.726640`, `0.722391`, `0.716555`, `0.719628`
- `n=5`, mean `0.709118`, std `0.024596`

**Interpretation**

- Mixed-site CV mean is **lower** than internal-only CV (`0.709` vs `0.734`), expected because the validation distribution is harder and includes cross-site variation.
- Mixed-site CV variance is **lower** (`0.0246` vs `0.0409`), likely because pooled folds smooth fold-to-fold case composition differences.
- This result does not contradict domain shift; it indicates the model still faces difficulty on a broader pooled distribution.

---

### Task 3 — Internal-only subset test (train/val/test all internal)

**Question this task answers**

- If we hold out an internal test subset (instead of external), does performance remain high?
- This isolates whether the dramatic drop is primarily external-domain specific.

**Protocol**

- New split builder: `scripts/build_internal_subset_split.py`
- Output split CSV: `reports/splits/internal_train_val_internal_test.csv`
- Train + internal test run: `slurm/train_internal_subset.slurm` (uses `--final-test`)
- Detailed post-hoc test eval: `scripts/monai_eval_segmentation.py --split test` on the same split/checkpoint.

**Observed output**

- Training summary (`internal_subset_01/summary.json`):
  - `best_val_mean_fg_dice = 0.6902934867474768`
- Internal test summary (`internal_subset_01/test_summary.json`):
  - `mean_fg_dice = 0.7246734318525895`, `n = 23`
- Detailed eval summary (`internal_subset_01_test_eval/eval_test_summary.md`):
  - Class 1 mean `0.7225 ± 0.1272`
  - Class 2 mean `0.7801 ± 0.2272`
  - Class 3 mean `0.6362 ± 0.1993`
  - Class 4 mean `0.7599 ± 0.1950`
  - Mean FG Dice `0.7247 ± 0.1645` (23/23 cases evaluated)
- Per-case CSV confirms outliers exist (e.g., a few very low mean-FG cases), but most cases are strong.

**Interpretation**

- Internal test (`0.725`) is much higher than prior external test (`0.297`), strongly supporting that the main gap is **domain shift**, not a general inability to segment internal anatomy.
- Class 3 (CPC) remains the weakest class on average versus classes 2 and 4, consistent with earlier observations and worth targeted analysis/post-processing.

---

### Consolidated takeaway from Tasks 1–3

- **Task 1** validated internal metric reliability: internal-only CV mean ~`0.734` with moderate fold spread.
- **Task 2** showed mixed-site pooled validation remains harder: mean ~`0.709`; lower variance but lower absolute score than internal-only CV.
- **Task 3** confirmed internal-domain test stays strong (~~`0.725`) while external baseline remained much lower (~~`0.297`), reinforcing external-domain shift as the core bottleneck.
